function [P] = get_BestMatch(labels, cl)
    %load([dataPath,'/labels.mat'])
    len_labels=length(labels);
    uni_labels=unique(labels);
    uni_cl=unique(cl);
    N=numel(uni_labels);
    H=numel(uni_cl);
    C=zeros(len_labels,N);
    G=zeros(len_labels,H);
    Omega=zeros(N,H);
    for i=1:len_labels
        for j=1:N
            if(labels(i)==uni_labels(j))
                C(i,j)=1;
            end
        end
    end
